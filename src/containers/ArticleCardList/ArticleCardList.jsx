...
const ArticleCardList = ({articleInfo}) => {
  const articleCardJSX = articleInfo.map((article, index) => {
    return <ArticleCard index={index} key={index} thumbnail={article.thumbnail} category={article.category} title={article.title} isActive={article.isActive} id={article.id} />;
  });
